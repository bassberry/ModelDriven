...
public class Pruefung2 extends JPanel {
	...
	public Pruefung2(AppUserInterface parent) {
		this.parent = parent;
		this.initialize();
	}
	...
	private void addWidgetsToScreen() {
		...
		pruefung2Panel_NamePanel.add(nameTextField);
		...
		// Eingabefeld in das Panel einfuegen
		pruefung2Panel_NotePanel.add(noteTextField);
...

		{
			...
			JButton proceedButton = new JButton("Beenden");
	...
	private void triggerNextPage() {
		try {
			...
			pruefung2.setName(nameTextField.getText());

			pruefung2.setNote(Integer.parseInt(noteTextField.getText()));

			// Objekt ins Hash schreiben
			DataStorage.INSTANCE.set("Pruefung2", pruefung2);

			parent.switchPage("Pruefung2");
		} catch (NumberFormatException exception) {
			// Integer-Konvertierungsfehler abfangen
			JOptionPane.showMessageDialog(Pruefung2.this, "Number could not be converted", "Number Format Exception",
